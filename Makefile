...
doc-api:
	sphinx-apidoc -f -d1 -T -e -M -t docs/_templates -o docs/api/ seastersdb
...
doc-serve: purge-server
	python -m http.server --directory docs/_build > /tmp/seastersdb_http.log 2>&1 & (sleep 2; python -m webbrowser "http://0.0.0.0:8000/")
